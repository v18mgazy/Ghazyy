...
export class RealtimeDBStorage implements IStorage {
   ...
   * يستخدم لتحديث بيانات التقارير بدلاً من حذفها وإعادة إنشائها
   * 
   * @param id معرف التقرير المراد تحديثه
   * @param reportData بيانات التقرير الجديدة
   */
  async updateReportData(id: number, reportData: Partial<ReportData>): Promise<ReportData | undefined> {
    try {
      console.log(`RealtimeDBStorage: Updating report data with ID ${id}`);
      const reportRef = ref(database, `report_data/${id}`);
      
      // التحقق من وجود التقرير قبل التحديث
      const snapshot = await get(reportRef);
      if (!snapshot.exists()) {
        console.warn(`RealtimeDBStorage: Report data with ID ${id} does not exist for update`);
        return undefined;
      }
      
      const currentReport = snapshot.val();
      
      // تحويل التواريخ إلى سلاسل نصية للتخزين في Firebase
      const updatedData: any = { ...reportData };
      if (updatedData.date && updatedData.date instanceof Date) {
        updatedData.date = updatedData.date.toISOString();
      }
      ...
      console.log(`RealtimeDBStorage: Successfully updated report data with ID ${id}`);
      
      // إرجاع البيانات المحدثة مع تحويل التواريخ إلى كائنات Date
      return {
        ...updatedReport,
        id,
        date: new Date(updatedReport.date),
        createdAt: new Date(updatedReport.createdAt),
        dataJson: updatedReport.dataJson || null,
      };
    } catch (error) {
      console.error(`RealtimeDBStorage: Error updating report data with ID ${id}:`, error);
      throw error;
    }
  }
}